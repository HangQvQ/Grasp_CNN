...
from grasp_det_seg.data_OCID import OCIDDataset, CornellDataset, iss_collate_fn, OCIDTransform, CornellTransform
from grasp_det_seg.models.det_seg import DetSegNet_OCID, DetSegNet_Cornell, NETWORK_INPUTS_OCID, NETWORK_INPUTS_Cornell
from grasp_det_seg.modules.fpn import FPN, FPNBody
from grasp_det_seg.modules.fusion import FusionModule
from grasp_det_seg.modules.heads import RPNHead, FPNSemanticHeadDeeplab, FPNROIHead
...
from grasp_det_seg.utils.snapshot import save_snapshot, resume_from_snapshot, pre_train_from_snapshots
...
def make_dataloader(args, config, rank, world_size):
    if args.dataset_name == 'OCID':
        ...
        # Training dataloader
        train_tf = CornellTransform(config.getint("shortest_size"),
                                config.getint("longest_max_size"),
                                config.getstruct("rgb_mean"),
                                config.getstruct("rgb_std"),
                                config.getboolean("random_flip"),
                                config.getstruct("random_scale"),
                                config.getstruct("rotate_and_scale"))
        train_db = CornellDataset(args.data, config["root_path"], config["train_set"], train_tf)
        train_sampler = DistributedARBatchSampler(
            train_db, config.getint("train_batch_size"), world_size, rank, True)
        train_dl = data.DataLoader(train_db,
                                batch_sampler=train_sampler,
        ...
        # Validation dataloader
        val_tf = CornellTransform(config.getint("shortest_size"),
                            config.getint("longest_max_size"),
                            config.getstruct("rgb_mean"),
                            config.getstruct("rgb_std")
                            )
        val_db = CornellDataset(args.data, config["root_path"], config["val_set"], val_tf)
        val_sampler = DistributedARBatchSampler(
            val_db, config.getint("val_batch_size"), world_size, rank, False)
        val_dl = data.DataLoader(val_db,
                                batch_sampler=val_sampler,
...
def make_optimizer(args, config, model, epoch_length):
    body_config = config["body"]
    ...
    if args.dataset_name == "OCID":
        parameters = [
            {
                ...
                "lr": opt_config.getfloat("lr_OCID") if not body_config.getboolean("bn_frozen") else 0.,
                "weight_decay": opt_config.getfloat("weight_decay") if opt_config.getboolean("weight_decay_norm") else 0.
            },
            {
                "params": other_parameters,
                "lr": opt_config.getfloat("lr_OCID"),
        ...
        parameters = [
            {
                "params": norm_parameters,
                "lr": opt_config.getfloat("lr_Cornell") if not body_config.getboolean("bn_frozen") else 0.,
                "weight_decay": opt_config.getfloat("weight_decay") if opt_config.getboolean("weight_decay_norm") else 0.
            },
            {
                "params": other_parameters,
                "lr": opt_config.getfloat("lr_Cornell"),
                "weight_decay": opt_config.getfloat("weight_decay")
            }
        ]        
...
def train(args, model, optimizer, scheduler, dataloader, meters, **varargs):
    model.train()
    dataloader.batch_sampler.set_epoch(varargs["epoch"])
    optimizer.zero_grad()
    global_step = varargs["global_step"]
    if args.dataset_name == 'OCID':
        loss_weights = varargs["loss_weights_OCID"]
    elif args.dataset_name == 'Cornell':
        loss_weights = varargs["loss_weights_Cornell"]
    ...
    for it, batch in enumerate(dataloader):
        # Upload batch
        if args.dataset_name == 'OCID':
            batch = {k: batch[k].cuda(device=varargs["device"], non_blocking=True) for k in NETWORK_INPUTS_OCID}
        elif args.dataset_name == 'Cornell':
            batch = {k: batch[k].cuda(device=varargs["device"], non_blocking=True) for k in NETWORK_INPUTS_Cornell}
            ...
            if args.dataset_name == 'OCID':
                logging.iteration(
                    ...
                    OrderedDict([
                        ...
                        ("roi_iou_loss", meters["roi_angle_loss"]),
                        ("sem_loss", meters["sem_loss"]),
                        ("data_time", data_time_meter),
                        ("batch_time", batch_time_meter)
                    ])
                )
            if args.dataset_name == 'Cornell':
                logging.iteration(
                    varargs["summary"], "train", global_step,
                    varargs["epoch"] + 1, varargs["num_epochs"],
                    it + 1, len(dataloader),
                    OrderedDict([
                        ...
                        ("roi_cls_loss", meters["roi_cls_loss"]),
                        ("roi_bbx_loss", meters["roi_bbx_loss"]),
                        ("roi_iou_loss", meters["roi_iou_loss"]),
                        ("data_time", data_time_meter),
                        ("batch_time", batch_time_meter)
                    ])
                ) 
...
def validate(args, model, dataloader, **varargs):
    model.eval()
    dataloader.batch_sampler.set_epoch(varargs["epoch"])
...
def main(args):
    ...
    snapshot_save_interval = 50  # 100 epochs
    ...
    if args.dataset_name == 'OCID':
        if args.resume:
            ...
            snapshot = resume_from_snapshot(model, args.resume, ["body", "rpn_head", "roi_head", "sem_head", 'fusion_module'])
        elif args.pre_train:
            ...
            pre_train_from_snapshots(model, args.pre_train, ["body", "rpn_head", "roi_head", "sem_head", 'fusion_module'])
        else:
    ...
    # Create optimizer
    optimizer, scheduler, batch_update, total_epochs = make_optimizer(args, config, model, len(train_dataloader))
    if args.resume:
    ...
    if args.dataset_name == 'OCID':
        meters = {
            ...
            "roi_iou_loss": AverageMeter((), momentum),
    ...
    for epoch in range(starting_epoch, total_epochs):
        ...
        # Run training epoch
        global_step = train(args, model, optimizer, scheduler, train_dataloader, meters,
                            ...
                            global_step=global_step, loss_weights_OCID=config["optimizer"].getstruct("loss_weights_OCID"),
                            loss_weights_Cornell=config["optimizer"].getstruct("loss_weights_Cornell"))
        ...
        if rank == 0:
            ...
            if args.dataset_name == 'OCID':
                save_snapshot(snapshot_file, config, epoch, 0, 0, global_step,
                            ...
                            fusion_module=model.module.fusion_module.state_dict(),
        ...
        if (epoch + 1) % config["general"].getint("val_interval") == 0:
            ...
            validate(args, model, val_dataloader,
                     loss_weights_OCID=config["optimizer"].getstruct("loss_weights_OCID"),
                     loss_weights_Cornell=config["optimizer"].getstruct("loss_weights_Cornell"),
            ...
            if (epoch + 1) % snapshot_save_interval == 0:
                if rank == 0:
                    shutil.copyfile(snapshot_file, path.join(os.path.join(args.log_dir, timestamp),
