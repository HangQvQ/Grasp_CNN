...
class OCIDTransform:
    ...
    def _prepare_frcnn_format(boxes, im_size):
        ...
        theta = np.zeros((A.shape[0]), dtype=np.int_)
        ...
        ret_value = (
        x_min[correct_idx], y_min[correct_idx], theta_deg[correct_idx], x_max[correct_idx], y_max[correct_idx], cls[correct_idx])
        return ret_value
    ...
    def __call__(self, img_, msk_, bbox_infos_):
        # im_size = [self.shortest_size, self.longest_max_size]
        im_size = [img_.shape[0], img_.shape[1]]
...
class OCIDTestTransform:
    ...
    def __call__(self, img):
        ...
        # Image transformations
        img_ = tfn.to_tensor(img)
        img_ = self._normalize_image(img_)

        return img_, im_size, img



class CornellTransform:
    """Transformer function for OCID_grasp dataset
    """
    ...
    def _prepare_frcnn_format(boxes, im_size):
        ...
        ret_value = (x_min[correct_idx], y_min[correct_idx], theta_deg[correct_idx], x_max[correct_idx], y_max[correct_idx],cls[correct_idx])
        return ret_value

    def _rotateAndScale(self, img, all_boxes):
        im_size = [self.shortest_size, self.longest_max_size]
        ...
        return imgRotateCrop, bbsInShiftBack

    def __call__(self, img_, bbox_infos_):
        im_size = [self.shortest_size, self.longest_max_size]
        bbox_infos_ = np.swapaxes(bbox_infos_, 1, 2)
        ...
        if self.rotate_and_scale:
            img, bbox_transformed = self._rotateAndScale(img, bbox_infos_)
            bbox_infos = bbox_transformed
        # Random flip
        if self.random_flip:
        ...
        return dict(img=img, bbx=bbx), im_size