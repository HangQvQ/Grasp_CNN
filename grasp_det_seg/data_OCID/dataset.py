...
class OCIDTestDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        img_, im_size, img_raw = self.transform(im_rgb)
...
class CornellDataset(data.Dataset):
    """Cornell_grasp dataset for grasp detection
    """
    def __init__(self, data_path, root_dir, split_name, transform):
        ...
        for root, dirs, files in sorted(os.walk(self.data_path)):
            for file in sorted(files):
                ...
                if file.endswith('.png'):
                    relative_dir = os.path.relpath(root, self.data_path)
    ...
    def _load_item(self, item):
        seq_path, im_name = os.path.split(item)
        sample_path = os.path.join(self.root_dir, seq_path)
        img_path = os.path.join(sample_path, im_name)
        anno_path = img_path.replace('r.png', 'cpos.txt')
        img_bgr = cv2.imread(img_path)
        img = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    ...
    def __getitem__(self, item):
        im_rgb, bbox_infos = self._load_item(item)
        if self.split_name == 'training' or self.split_name == 'validation':
            rec, im_size = self.transform(im_rgb, bbox_infos)
            rec["abs_path"] = item
            rec["root_path"] = self.root_dir
            rec["im_size"] = im_size
            return rec
