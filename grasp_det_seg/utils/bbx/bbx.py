...
def shift_boxes_fpn(bbx, shift, dim=-1, scale_clip=log(1000. / 16.)):

    shift_ = shift[:, :, [0,1,2,3]]
    angle = shift[:, :, 4]
    yx_in, hw_in = corners_to_center_scale(*bbx.split(2, dim=dim))
    dyx, dhw = shift_.split(2, dim=dim)
    yx_out = yx_in + hw_in * dyx
    hw_out = hw_in * dhw.clamp(max=scale_clip).exp()
    yxhw = torch.cat(center_scale_to_corners(yx_out, hw_out), dim=dim)
    return torch.cat([yxhw, angle.unsqueeze(-1)], dim=dim)
...
def ious_r(bbx0, bbx1):

    bbx0_tl, bbx0_br = bbx0[:, :2], bbx0[:, 2:]
    bbx1_tl, bbx1_br = bbx1[:, :2], bbx1[:, 2:]
