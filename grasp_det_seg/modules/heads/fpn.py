...
class FPNROIHead(nn.Module):
    ...
    def __init__(self, in_channels, classes, roi_size, hidden_channels=1024, norm_act=ABN):
        ...
        self.fc2 = nn.Sequential(OrderedDict([
            ("fc3", nn.Linear(int(roi_size[0] * roi_size[1] * in_channels / 4), hidden_channels, bias=False)),
            ("bn3", norm_act(hidden_channels)),
            ("fc4", nn.Linear(hidden_channels, hidden_channels, bias=False)),
            ("bn4", norm_act(hidden_channels))
        ]))
        
        self.roi_cls = nn.Linear(hidden_channels, classes["thing"] + 1)
        self.roi_bbx = nn.Linear(hidden_channels, classes["thing"] * 4)
        self.roi_iou = nn.Linear(hidden_channels, classes["thing"])

        self.reset_parameters()

    def reset_parameters(self):
        gain = nn.init.calculate_gain(self.fc1.bn1.activation, self.fc1.bn1.activation_param)
    ...
    def forward(self, x):
        ...
        return self.roi_cls(x1),  self.roi_bbx(x1).view(x.size(0), -1, 4), self.roi_iou(x2)
