...
from grasp_det_seg.utils.bbx import shift_boxes, shift_boxes_fpn
...
class DetectionAlgoFPN(DetectionAlgo):
    ...
    def training(self, head, x, proposals, bbx, cat, iscrowd, valid_size, img_size):
        x = x[self.min_level: self.min_level + self.levels]
        #fp = fp[self.min_level: self.min_level + self.levels]
        try:
            ...
            with torch.no_grad():
                ...
                cls_lbl, bbx_lbl, iou_lbl = self._match_to_lbl(proposals, bbx, cat, match)

            # Run head
            set_active_group(head, active_group(True))
            proposals, proposals_idx = proposals.contiguous
            cls_logits, bbx_logits, iou_logits = self._head(head, x, proposals, proposals_idx, img_size)

            # Calculate loss
            bbx_loss, cls_loss, iou_loss = self.loss(cls_logits, bbx_logits, iou_logits, cls_lbl, bbx_lbl, iou_lbl)
            ...
            boxes, scores, ious = self._split_and_clip(boxes, scores, iou_logits, proposals_idx, valid_size)
            bbx_pred, cls_pred, obj_pred, iou_pred = self.prediction_generator(boxes, scores, ious)

        except Empty:
            active_group(False)
            cls_loss = bbx_loss = iou_loss= sum(x_i.sum() for x_i in x) * 0
            ...
            iou_pred = PackedSequence([None for _ in range(x[0].size(0))])

        return cls_loss, bbx_loss, iou_loss, bbx_pred, cls_pred, obj_pred, iou_pred
    ...
    def inference(self, head, x, proposals, valid_size, img_size):
        ...
        return bbx_pred, cls_pred, obj_pred, iou_pred