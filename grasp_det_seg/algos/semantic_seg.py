...
class SemanticSegAlgo:
    ...
    def _logits(head, x, valid_size, img_size):
        ...
        for factor in factors:
            new_height = img_size[0]//factor
            new_width = img_size[1]//factor
            new_size = (new_height, new_width)
            si = SemanticSegAlgo._pack_logits(sem_logits, valid_size, new_size)
            s.append(si)
        return sem_logits, SemanticSegAlgo._pack_logits(sem_logits, valid_size, img_size), sem_feats, s
    ...
    def training(self, head, x, sem, valid_size, img_size):
        ...
        sem_logits_low_res, sem_logits, sem_feats, s = self._logits(head, x, valid_size, img_size)
        sp = []
        for i in range(4):
            si = PackedSequence([sem_logits_i.max(dim=0)[1] for sem_logits_i in s[i]])
            sp.append(si)
        ...
        return sem_loss, conf_mat, sem_pred, sem_logits, sem_logits_low_res,sem_pred_low_res,sem_feats, sp
    ...
    def inference(self, head, x, valid_size, img_size):
        ...
        return sem_pred, sem_feats, sem_logits, sem_pred_low_res, sp
