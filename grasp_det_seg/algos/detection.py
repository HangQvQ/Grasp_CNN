...
from grasp_det_seg.utils.bbx import ious, ious_r, calculate_shift, bbx_overlap, mask_overlap
...
class PredictionGenerator:
    ...
    def __call__(self, boxes, scores, ious):
        """Perform NMS-based selection of detections
        ...
        obj_pred : PackedSequence
            ...
            detection can be kept according to the selection parameters
        """
        bbx_pred, cls_pred, obj_pred, iou_pred = [], [], [], []
        for bbx_i, obj_i, iou_i in zip(boxes, scores, ious):
            try:
                if bbx_i is None or obj_i is None is None:
                    raise Empty

                # Do NMS separately for each class
                bbx_pred_i, cls_pred_i, obj_pred_i, iou_pred_i = [], [], [], []
                for cls_id, (bbx_cls_i, obj_cls_i, iou_cls_i) in enumerate(zip(torch.unbind(bbx_i, dim=1),
                                                                    torch.unbind(obj_i, dim=1)[1:],
                                                                    torch.unbind(iou_i, dim=1)[1:])):
                ...
                iou_pred_i = torch.cat(iou_pred_i, dim=0)
                ...
                if bbx_pred_i.size(0) > self.max_predictions:
                    ...
                    iou_pred_i = iou_pred_i[idx]

                # Save results
                bbx_pred.append(bbx_pred_i)
                cls_pred.append(cls_pred_i)
                obj_pred.append(obj_pred_i)
                iou_pred.append(iou_pred_i)
            except Empty:
                bbx_pred.append(None)
                cls_pred.append(None)
                obj_pred.append(None)
                iou_pred.append(None)

        return PackedSequence(bbx_pred), PackedSequence(cls_pred), PackedSequence(obj_pred), PackedSequence(iou_pred)
...
class DetectionLoss:
    ...
    def iou_loss(self, iou_logits, iou_lbl, num_non_void):
        iou_loss = smooth_l1(iou_logits, iou_lbl, self.sigma).sum()
        iou_loss /= num_non_void
        return iou_loss

    def __call__(self, cls_logits, bbx_logits, iou_logits, cls_lbl, bbx_lbl, iou_lbl):
        """Detection loss
        """
        # Get contiguous view of the labels
        cls_lbl, _ = cls_lbl.contiguous
        iou_lbl, _ = iou_lbl.contiguous
        ...
        if positives.any().item():
            ...
            iou_logits = iou_logits[positives]
            ...
            iou_loss = self.iou_loss(iou_logits[idx, cls_lbl - 1], iou_lbl, num_non_void)
        else:
            bbx_loss = bbx_logits.sum() * 0
            iou_loss = iou_logits.sum() * 0

        return bbx_loss, cls_loss, iou_loss
...
class DetectionAlgo:
    ...
    @staticmethod
    def _split_and_clip(boxes, scores, iou_logits, index, valid_size):
        boxes_out, scores_out, iou_logits_out = [], [], []
        for img_id, valid_size_i in enumerate(valid_size):
            ...
            if idx.any().item():
                ...
                iou_logits_i = iou_logits[idx]

                boxes_out.append(boxes_i)
                scores_out.append(scores[idx])
                iou_logits_out.append(iou_logits_i)
            else:
                boxes_out.append(None)
                scores_out.append(None)
                iou_logits_out.append(None)

        return boxes_out, scores_out, iou_logits_out
        ...
        for i, (proposals_i, bbx_i_, match_i) in enumerate(zip(proposals, bbx, match)):
            ...
            if match_i is not None:
                ...
                if pos.any().item():
                    ...
                    # iou labels
                    iou_lbl_i = ious_r(proposals_i[pos], bbx_i[match_i[pos]])
                else:
                    bbx_lbl_i = None
                    iou_lbl_i = None
                ...
                iou_lbl.append(iou_lbl_i)
            else:
                cls_lbl.append(None)
                bbx_lbl.append(None)
                iou_lbl.append(None)

        return PackedSequence(cls_lbl), PackedSequence(bbx_lbl), PackedSequence(iou_lbl)
